 ...
 * @version $Id: index.php 286 2009-09-19 20:18:05Z xycaleth $
 *
...
else if ( $id == 'login' )
{
    ...
    $post_user = ( isset ($VARS['username']) ) ? substr ($VARS['username'], 0, 40) : '';
    $post_pass = ( isset ($VARS['password']) ) ? substr ($VARS['password'], 0, 40) : '';
...
else if ( $id == 'admin_paths' )
{
    ...
    echo "<form action=\"?id=admin_paths_save\" method=\"post\">\n",
         ...
         "<input type=\"text\" class=\"post\" id=\"site1\" name=\"site1\" value=\"$site\" size=\"80\" /><br />\n",
         "$ind48\n",
         "</fieldset>\n",

         "<fieldset>\n",
         "<legend><label for=\"furl1\">$ind49</label></legend>\n",
         "<input type=\"text\" class=\"post\" id=\"furl1\" name=\"furl1\" value=\"$furl\" size=\"80\" /><br />\n",
         "$ind50\n",
         "</fieldset>\n",

         "<fieldset>\n",
         "<legend><label for=\"url\">$ind53</label></legend>\n",
         "<input type=\"text\" class=\"post\" id=\"url\" name=\"url\" value=\"$hurl\" size=\"80\" /><br />\n",
         "$ind54\n",
         "</fieldset>\n",
...
else if ( $id == 'admin_news' )
{
    ...
    echo "<form action=\"?id=admin_news_save\" method=\"post\">\n",
         ...
         "<input type=\"checkbox\" class=\"post\" id=\"news_pagination\" onchange=\"toggle_pagination_options();\" name=\"news_pagination\" value=\"1\"" . checkbox_checked ($news_pagination) . " /> <label for=\"news_pagination\">$ind332</label>",
...
<textarea name="edit1" id="edit1" class="post" cols="85" rows="20">$edit1</textarea><br />
...
html;
    if ( $name2 )
    {
        ...
        echo <<<html
<div style="text-align:center">
<textarea name="edit2" id="edit2" class="post" cols="85" rows="20">$edit2</textarea><br />
...
else if ( $id == 'user_update')
{
    ...
    $nick1 = ( isset ($VARS["nick1"]) ) ? substr ($VARS['nick1'], 0, 40) : '';
    $mail1 = ( isset ($VARS["mail1"]) ) ? $VARS['mail1'] : '';
    $new_password = ( isset ($VARS['new_password']) ) ? substr ($VARS['new_password'], 0, 40) : '';
    $confirm_pass = ( isset ($VARS['confirm_pass']) ) ? substr ($VARS['confirm_pass'], 0, 40) : '';
    $icon1 = ( isset ($VARS["icon1"]) ) ? $VARS['icon1'] : '';
    $timeoffset = ( isset ($VARS["timeoffset"]) ) ? (int)$VARS['timeoffset'] : '';
    $name = ( isset ($VARS["name"]) ) ? substr ($VARS['name'], 0, 40) : '';
...
else if ( $id == 'updateprofile')
{
    ...
    $email = ( isset ($VARS["mail1"]) ) ? $VARS['mail1'] : '';
    $oldpassw = ( isset ($VARS["oldpassw"]) ) ? substr ($VARS['oldpassw'], 0, 40) : '';
    $passw = ( isset ($VARS["passw"]) ) ? substr ($VARS['passw'], 0, 40) : '';
    $nick = ( isset ($VARS['nick1']) ) ? substr($VARS["nick1"], 0, 40) : '';
...
elseif ( $id == 'postnews' )
{
    ...
    $news = ( isset ($VARS['news']) ) ? $VARS['news'] : '';
    $categories = ( isset ($VARS['category']) ) ? $VARS['category'] : array(1);
    $fullnews = ( isset ($VARS['fullnews']) ) ? $VARS['fullnews'] : '';
    $subject = ( isset ($VARS['post_subject']) ) ? $VARS['post_subject'] : '';
    $description = ( isset ($VARS['description']) ) ? $VARS['description'] : '';

    if ( $action == 'preview' )
    {
        $news1 = format_message ($news, $ht, $bb, $wfpost, $smilies);
        $fullnews1 = format_message ($fullnews, $ht, $bb, $wfpost, $smilies);
...
html;
    ...
    $extras = show_extras ('newsposting', 'news', $smilies, $bb);
	...
	<tr>
		<td align="center">
			$extras
			<textarea class="post" id="news" name="news" rows="15" cols="80">$news</textarea>
		</td>
	</tr>
</table>
<p></p>
...
html;
    $extras = show_extras ('newsposting', 'fullnews', $smilies, $bb);
	...
	<tr>
		<td align="center">
			$extras
			<textarea class="post" id="fullnews" name="fullnews" rows="15" cols="80">$fullnews</textarea>
		</td>
	</tr>
</table>
<p>
...
else if ( $id == 'editposts2' )
{
    ...
    if ( $action == 'preview' )
    {
        $shortnews = ( isset ($VARS['news']) ) ? $VARS['news'] : '';
        $fullnews = ( isset ($VARS['fullnews']) ) ? $VARS['fullnews'] : '';
        $description = ( isset ($VARS['description']) ) ? $VARS['description'] : '';
        ...
        $preview_shortnews = format_message ($shortnews, $ht, $bb, $wfpost, $smilies);
        $preview_fullnews = format_message ($fullnews, $ht, $bb, $wfpost, $smilies);
...
html;
    ...
    echo <<<html
<form action="?id=editposts3" method="post" id="newsposting" onsubmit="submitonce(this);">
<table class="adminpanel">
	<tr>
		<td>$ind119</td>
		<td>$writer</td>
...
html;
	...
	<tr>
		<td align="center">
			$extras
			<textarea class="post" id="news" name="news" rows="15" cols="70">$shortnews</textarea>
		</td>
	</tr>
</table>
<p></p>
...
html;
	...
	<tr>
		<td align="center">
			$extras
			<textarea class="post" id="fullnews" name="fullnews" rows="15" cols="70">$fullnews</textarea>
		</td>
	</tr>
</table>
<p>
...
<<< html
	<tr>
		...
		<td><a href="?id=editcomments&amp;rand={$article['news_id']}">{$article['headline']}</a></td>
...
{ /*id Comments Edit*/
    ...
    $rand = ( isset ($VARS['rand']) ) ? (int)$VARS['rand'] : 0;
    ...
    echo <<< html
				$ind138<br />
				<form method="post" id="comments" action="?id=delcomments">
				<table class="adminpanel">
					<tr>
...
html;
	...
	<tr>
		...
		<td>$com_datum</td>
		<td style="text-align:center">
			[<a href="?id=editcomment&amp;num={$comment['comment_id']}&amp;rand=$rand">$ind30b</a>]
		</td>
	</tr>
html;
...
{ /*id Comment Edit*/
    ...
    $comment_id	= ( isset ($VARS['num']) ) ? (int)$VARS['num'] : 0;
    ...
    foreach ( $file as $value )
    {
        ...
        if ( $comment_id == $comment['comment_id'] )
        {
            ...
            $title = $ind134;
            echo <<< html
<form action="?id=updatecomment&amp;num={$comment['comment_id']}&amp;rand={$news_id}" method="post" id="newsposting" onsubmit="submitonce(this);">
<table class="adminpanel">
	<tr>
		<td>$ind279</td>
...
html;
	...
	<tr>
		<td colspan="3">
			...
			<textarea class="post" name="comment" id="comment" rows="15" cols="70">$message</textarea></div>
...
{ /*id Comment Update*/
    ...
    $comment_id = ( isset ($VARS['num']) ) ? $VARS['num'] : 0;
    $news_id = ( isset ($VARS['rand']) ) ? (int)$VARS['rand'] : 0;
    $del = isset ($VARS['del']);
...
{/*id Smillies Add*/
    ...
    $smiley_image = ( isset ($VARS['smiley_image']) ) ? $VARS['smiley_image'] : '';
    ...
    {
        trigger_error ($ind201, E_USER_WARNING);
    }
    
    $random = mt_rand();
    $smiley_image = trim ($smiley_image, "\n..\r");
...
{/*id Smillies Edit*/
    ...
    foreach ( $code_smillie as $rand_id => $code )
    {
        ...
        $smilies .= $rand_id . '|<|' . $code . '|<|' . $smiley_image[$rand_id] . '|<|' . "\n";
    }
